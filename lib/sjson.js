 ...
 * 99.9% of this code is based on https://github.com/bestiejs/json3. Thank you Kit Cambridge.
 */
;(function () {
  // Convenience aliases.
  ...
  if (!stringifySupported || !parseSupported) {
    ...
    if (!stringifySupported) {
          ...
          if (className == "[object Array]") {
            ...
            forEach(properties || value, function (property) {
              ...
              if (element !== undef) {
                ...
                // `JSON.stringify`.
                property_strings.push([property, quote(property) + ":" + (whitespace ? " " : "") + element]);
              }
              any || (any = true);
            });
            property_strings.sort(function (a, b) {
              if(a[0] > b[0]) return 1;
              if(a[0] < b[0]) return -1;
              return 0;
            });
            property_strings.forEach(function (strings) {
              results.push(strings[1]);
            });
            return any ? (whitespace ? "{\n" + indentation + results.join(",\n" + indentation) + "\n" + prefix + "}" : ("{" + results.join(",") + "}")) : "{}";
          }
      ...
      SJSON.stringify = function (source, filter) {
        var width = 4;
    ...
    if (!parseSupported) {
      ...
      SJSON.parse = function (source, callback) {
        Index = 0;
        Source = source;
        var result = get(lex());
